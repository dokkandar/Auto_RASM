...
**▶ RESUME HERE (2026-06-11): next = T-junction (#2)**, then X-crossing / collinear. Also owed on the wall-style track: convert-closed-dobject→wall, true hatch patterns for poché (currently a solid tint), curved-wall fill + curved-wall snap/grips (curved walls render + transform but editing is straight-approx). Build GREEN; today's wall work (incl. WallStyle + curved fillet) is UNCOMMITTED (last commit fe00208 = dimensions).
