...
### BUG FIX: CARD didn't apply during scale (2026-06-12, uncommitted)
- [x] User: CARD not working in scale. Cause: `card_anchor()` had no `scale_state` arm, so `apply_constraints` had no anchor during scale picks → CARD no-op. Added: WaitingForFactor/WaitingForNewLength → pivot; WaitingForRefEnd → ref_start (each click measures distance from that anchor, so CARD locks it to H/V). Scale WaitingForFactor preview now uses `cursor_world_constrained` so the ghost + factor readout match the CARD-locked commit. (Dump's `r → ParseErr` was a recorder cosmetic — the scale `r`/`c` intercept at app.rs:2351 DID fire; reference-mode scale completed.)
...
- **Ribbon + icons (2026-06-13, uncommitted)**: added a **Blocks group** to the 2nd toolbar row (after Inquiry) — three painted icon buttons: `block` (frame + base-point grip), `insert` (square dropping onto a node), `explode` (core square bursting into shards). New `GlyphKind::{Block,Insert,Explode}` + `draw_cmd_glyph` arms. `cmd_button` refactored to delegate to new `cmd_button_resp` (returns `egui::Response`) so the **insert** button can hang a `popup_below_widget` dropdown of defined block names (empty → bare `insert` prints the hint). **Explode added to the Modify menu** (next to Array…) — it was previously command/toolbar-only with no menu entry. Build clean.

### Parametric blocks — Objective 2 SLICE 1: block-diff rule extraction (2026-06-16, uncommitted)
Programming-by-demonstration: compare two near-identical block definitions (door @1000 vs @1150) → propose the parametric rule. Pure kernel module `cad_kernel/src/blockdiff.rs`: `diff_blocks(a, a_base, b, b_base, eps) -> BlockDiff`. Algorithm: build a base-relative `Feat{tag,pts,scal}` per dobject (Line=2pts, Polyline=verts+bulges+closed, Circle=center+r, Arc=endpoints+bulge, Point=loc, else bbox corners) → Pass 1 exact-match = unchanged majority → Pass 2 same-shape nearest-centroid = changed pairs → per-point deltas (>eps) → CLUSTER by displacement vector (same vector = one `ParamCluster{win,dir,magnitude,point_count}` = one stretch step; symmetric ±moves = 2 clusters, user merges later). 4 kernel tests (identical/one-edge/base-translation/rectangle). App: `blockdiff <A> <B>` / `bdiff` command → `run_block_diff` resolves both blocks by name, auto-scales eps to block extent, prints matched/changed counts + each candidate parameter (magnitude+dir+window+point count) to the command log AND a recorder Note (copyable). NO rule persistence/runtime yet — extraction only (validates the rule before building authoring UI). This is the NO-solver objective; the constraint-solver objective (O1) is a separate later track. Build clean; 17 app + 165 kernel tests pass.
**+ HARDENED after first real test (2026-06-16):** first run (`blockdiff d100 d90`) returned 38 "parameters" — 2 real (7/11 pts, clean axes) buried in 36 single-point noise. Two causes: greedy nearest-centroid MIS-MATCHES when the blocks aren't a clean copy+stretch, and non-uniform changes (swing-arc reshape / rotated parts) produce per-point deltas that can't cluster. Fixes: (1) **index-first correspondence** — when counts equal + all same-shape + ≥50% identical (the copy-then-edit case), pair i↔i exactly (no scramble), else fall back to exact-then-nearest; (2) clusters **sorted by point_count desc**; (3) app splits **strong params (≥2 pts)** from **1-point outliers** (reported as a count + "if many, use the ✂ stretch recorder instead"). Dropped a distance-gate idea (mis-pairs are intra-block so it only rejected real moves). 5 kernel tests. LESSON: diffing two FINISHED blocks is an under-determined inverse problem for curved/rotated parts — the stretch RECORDER is the reliable authoring path; diff is the convenience path for clean rigid-stretch variants.

**+ btr per-stretch NAMING + return-to-app (2026-06-16, uncommitted):** user: "after stretch there is no return to the app; this record should get a name for the function; the amount is taken dynamically at insert." Fix: after each btr stretch, `apply_stretch` sets `btr_awaiting_name` + prompts "NAME this function (Enter to skip)" — a run_command intercept stores the typed name on the last recorded ParamRow, then re-prompts "STRETCH again or finish". `finish`/`endrec` at the name prompt ends; `stretch`/`st`/`s` falls through to record another (unnamed). So control returns to the recorder after every stretch and each function gets a name on the spot; the displacement amount stays dynamic (= entered value − original at insert). The finish dialog pre-fills these names. Build clean; 17 app + 166 kernel tests pass.

**+ BUG FIX: btr sandbox overlapped the original instance (2026-06-16, uncommitted):** the recorder exploded the block to a sandbox but LEFT the block's own instance(s) in the doc at the same spot (tell: index showed 47 = 45 sandbox + d100 + d90 instances). So the stretch crossing-window grabbed the original `BlockRef` too → stretching it moved the whole original door AND polluted the recorded task with the BlockRef's bbox points (24 pts for a 14-dobject pick). Fix: `begin_block_task_recorder` now REMOVES all instances of the source block for the session (stored in `BlockTaskRec.removed_instances`) and finish/Esc-cancel re-add them. Build clean; 17 app + 166 kernel tests pass.

**+ BLOCK TASK RECORDER — forward-recording authoring (2026-06-16):** user's key insight — the block-DIFF is a LOSSY inverse problem (can't tell an intended param from a side-effect; e.g. P6 moved only because another stretch dragged it). The recorder captures the FORWARD ops = reversible. Flow (user decided: temp sandbox discarded + name-all-at-finish): `btr`/`blocktask` (or the Block dialog's "Block Task Recorder ▶" button) on a SELECTED block instance → explode it into a temp sandbox (tracked by handle, placed at the instance via `cd.geom.translated(insert-base)`) → each `stretch` the user demonstrates is recorded in `apply_stretch` as a `ParamRow` (window+dir+affected points, base-relative via `world_offset=insert`) → `finish`/`endrec` deletes the sandbox + opens the same "Set parameters on block" dialog with the recorded tasks to name/link/Save. Esc cancels (sandbox discarded). New `BlockTaskRec` state, `geom_def_points` helper, parser `BlockTaskRecorder`/`BlockTaskFinish`. Diff (Compare) kept as the secondary/lossy path. **Also fixed a latent bug:** parametric windows were stored base-relative but applied to definition-space geometry → params only worked when base==0; `save_block_params` now adds the block base. Build clean; 17 app + 166 kernel + 17 io tests pass.

**+ MODEL corrected per user's spec table (2026-06-16):** displacement is NOT a two-sample interpolation — it's `dir * (entered_value − original)`. Each variable = `BlockParam{name, original, vectors:Vec<ParamVector{win,dir}>}` (dropped target_value/measured-scaling; `original` = the source block's value, e.g. 1000). At insert the user enters V; every linked vector displaces by `dir·(V − original)` (value is in geometry units, 1:1). Dialog columns now: P · Δx,Δy · name · original (single field, was src/tgt). NOTE: `dir` is the source→target diff direction, so the sign depends on pick order — if a door shrinks when it should grow, swap source/target. Build clean; 17 app + 166 kernel + 17 io tests pass.

**+ Variable LINKING + insert-time PROMPT (2026-06-16):** kernel model now two-level — `BlockParam{name,source_value,target_value,vectors:Vec<ParamVector>}` (a named VARIABLE owns ≥1 linked modifier vectors) + `ParamVector{win,dir,measured}`; `frac(value)` linear map. `save_block_params` groups dialog rows BY NAME → rows with the SAME name LINK into one variable (move together, one insert prompt). `block_derived_geoms` loops variable→frac→each vector's stretch. INSERT-TIME PROMPT: `apply_insert` on a parametric block starts `InsertParamPrompt` (asks "set width [1000]:" per variable on the command line; pre-parser intercept reads each; Enter=default; Esc cancels; places once all entered) instead of placing immediately. Dialog note: "same name = linked." Per-instance editing still also available via Properties. Build clean; 17 app + 166 kernel + 17 io tests pass.

**+ WORKING PARAMETRIC BLOCK — full loop (2026-06-16):** the block-diff is now wired end-to-end (user opted into save+insert-derive). KERNEL: `BlockParam{name,win_min,win_max,dir,measured,source_value,target_value}` + `displacement(value)=dir*measured*(value-src)/(tgt-src)`; `Block.params:Vec<BlockParam>`; `BlockRef.param_values:[f64;MAX_BLOCK_PARAMS=8]` (fixed array keeps BlockRef Copy). RSM defaults them (not persisted yet); geom.rs transforms carry param_values via `..*br`. DERIVE: single `block_derived_geoms(blk,param_values)` (param = `stretch_one` over its window by `dir·displacement`); routed through draw_blockref + resolved_blockref_bbox + expand_cutter_geoms (trim/snap) + apply_explode → render/bbox/trim/snap/explode all show the derived geometry. AUTHOR: dialog "Set parameters on block" — pick source then target on screen (`BlockDiffPick`), `open_param_name_dialog` extracts strong clusters → `ParamNameDialog` with a row per modifier vector (Δx,Δy + name + src/tgt value fields) → `save_block_params` writes `BlockParam`s onto the SOURCE block (marks smart; resets existing instances to source values). EDIT/derive: Properties dialog BlockRef arm lists each param name + a DragValue → set value → geometry derives live. Insert sets param_values to source defaults. LIMIT: params not persisted to RSM yet; value entry is via Properties (not insert-time prompt). Build clean; 17 app + 166 kernel + 17 io tests pass.

**+ User-feedback revisions (2026-06-16):** (1) report each modifier vector as **Δx, Δy** components (was magnitude+unit-dir) — `[P1] Δx=… Δy=…`; output reframed as "base block '{A}' + N modifier vector(s)". (2) **Pick blocks on screen** instead of dropdowns: bare `blockdiff` (or the dialog's "Pick 2 blocks to compare ▶" button) → `BlockDiffPick{Off,WaitingForA,WaitingForB(id)}` state; click BASE block, click variant → `run_block_diff_ids(ia,ib)`. `Command::BlockDiff` now `Option<(String,String)>` (None=pick). Added to `in_click_only_phase`; Esc cancels. Dialog dropdowns + `BlockDialog.compare_a/b` removed. (3) **Overlay BASE-block-only** — the floating dots in the screenshot were block A's base-relative points wrongly drawn on block B's instance; now only block_a's instances are highlighted. `run_block_diff` split into name-resolver + `run_block_diff_ids` core.

**+ Parametric-point HIGHLIGHT overlay (2026-06-16):** after a compare, the strong clusters' moved points are drawn on EVERY placed instance of either block, colour-coded per parameter (P1 red, P2 cyan, …) with a per-cluster direction arrow + "P#" label. `ParamCluster.points` now carries the base-relative before-positions; `BlockDiffOverlay{block_a,block_b,clusters:Vec<(points,dir)>}` stored on CadApp, set in `run_block_diff` (strong clusters only); rendered in the canvas after grips by mapping each base-relative point through every matching instance's transform (insert + R·scale). Cleared on Esc / `clear` / next compare. NOTE: typing `width` errors ("unknown command") — naming/merge clusters → inputs is the NEXT slice (Slice 2).

**+ Block dialog "Compare blocks → parametric rule" section (2026-06-16):** when ≥2 blocks exist, the Block dialog shows two block dropdowns (A/B, persisted on `BlockDialog.compare_a/compare_b`) + a "Compare ▶" button (enabled only when A≠B both picked) → runs `run_block_diff`; dialog stays open, result prints to log + recorder Note. Same engine as the `blockdiff` command, now discoverable in the UI.

### TOOL: stretch recorder for smart-block authoring (2026-06-13, uncommitted)
Step 0 of parametric/semi-smart blocks (user's "record stretches → extract the function" plan). While the Session Recorder runs, every `stretch` (incl. DDE-stretch) now emits a `DbgEvent::StretchRecord` with the BOX (win_min→win_max), the VECTOR (base→dest, |v|, unit dir), and one block PER CHANGED dobject = idx + handle + kind + verbose before/after coordinates (`describe_verbose`, filtered to dobjects whose coords actually changed). Captured in `apply_stretch` only when `self.dbg.recording`. Copy the recorder timeline → the dump has everything needed to derive each parametric stretch step (which dobjects move, by what vector, inside which box). NOT the smart-block runtime yet — this is the authoring/data-capture instrument. Build clean; 17 app tests pass.
**+ "📐 Capture smart dobject" button (2026-06-13):** new button in the Session Recorder window (under a "Smart dobject authoring" section) → `capture_smart_geometry()` snapshots the FULL geometry of the current selection into the timeline as `DbgEvent::GeometryCapture` — an exploded SET (each dobject's `describe_verbose` coords) OR a BLOCK (also dumps the block name/base/insert + every DEFINITION-space content dobject). This is the BASE geometry; paired with the `✂REC` stretch steps it's everything to convert into a parametric smart block. Requires recording active + a non-empty selection (else a history hint).

### Stretch: Direct Distance Entry + arc-endpoint reshape (2026-06-13, uncommitted)
**(1) DDE for stretch** — typing a number during the dest pick now drops the destination along the cursor/CARD direction from the base, same as move/copy (dump showed `5` → `ParseErr` — the DDE intercept had no stretch arm). Added `StretchState::WaitingForDest(_,_,base)` to both the `dde_anchor` match and the apply cascade (`apply_stretch(wmin,wmax,base,p)`). CARD direction was ALREADY wired via `card_anchor`'s WaitingForDest arm, so with CARD on, "5 ⏎" stretches exactly 5 units along the locked H/V axis.
**(2) Arc-endpoint stretch (critical)** — `stretch_one` only handled `Arc if inside(center)` (translate whole arc); an arc whose ENDPOINT was in the window didn't move. Now: center-in → translate; else endpoint-in → move that endpoint to end+v and rebuild via `bulge_arc(ns, ne, tan(sweep/4))` (preserves curvature + CCW; the moved end lands exactly at the new location; both-in = pure translation; degenerate → unchanged). Uses `Arc::endpoints()`. Build clean; 17 app tests pass.

### Object-snap snaps THROUGH blocks (snap-through) (2026-06-13, uncommitted)
osnap (END/MID/CEN/…) ignored geometry inside block instances — `dist` + `end` over a door block captured the raw cursor, not an interior endpoint. The kernel's `BlockRef` snap offers only the insertion point (it can't reach `Document.blocks`). Fix (same in-memory-explode family as the trim-cutter + invisible-instance fixes): new `block_snap_phantoms()` explodes every VISIBLE block's contents into temporary `DObject`s (reuses `expand_cutter_geoms`, nested-safe), and the snap-candidate assembly runs a THIRD `find_all_snaps` pass over them — merged + re-sorted by (priority, distance) alongside the doc snaps and the pline phantom. Document untouched. Marker + `click_world` then land on the interior point. Expands all visible blocks per hover frame (find_all_snaps filters by radius); viewport-gate later if it profiles hot. Build clean; 17 app tests pass. **Block in-memory-explode now covers: render-cull bbox, trim/extend cutters, AND snap.**

### Trim/Extend can cut against geometry INSIDE a block (2026-06-13, uncommitted)
User wanted a normal line trimmed using the lines/arcs that live inside a block instance (door symbol as cutter). Failed with "target has no intersection with the cutting edges" because a `BlockRef` cutter was cloned verbatim into `cutter_geoms` and `BlockRef::intersect` is empty — the math couldn't see inside. Implemented the user's algorithm (in-memory explode → temp memo → do the math, doc untouched): new `expand_cutter_geoms(g, out, depth)` flattens a cutter/boundary dobject into its EFFECTIVE geometry — a BlockRef is exploded by transforming each contained dobject into world space (recursive for nested blocks, depth cap 8, same as draw_blockref), everything else passes through. Wired into BOTH `apply_trim_pick` (cutter_geoms) and `apply_extend_pick` (boundary_geoms). The block stays a block; only a throwaway Vec<Geom> is built per pick. fillet/chamfer/offset NOT changed (they operate ON a picked object, not against a basket — picking an edge inside a block is a separate feature). Build clean; 17 app tests pass.

### BUG FIX: block instances INVISIBLE (created blocks vanish + inserts don't show) (2026-06-13, uncommitted)
Two reported symptoms, one root cause: a `BlockRef`'s KERNEL bbox is a degenerate point at the insertion (the kernel can't resolve the block table), so in the CPU render loop `bbox_px = 0` tripped the **sub-pixel micro-cull** (`if bbox_px < 1.0 && !selected …`) → every UNselected instance was skipped and never drawn. That's why (1) `block` create made geometry "disappear" (originals replaced by ONE unselected instance → culled) and (2) `insert` placed an instance that was pickable (`hit_release=Some(idx)`, hit-test uses the resolved bbox) but invisible. Hatch was already short-circuited above the cull for the same reason; BlockRef was not. Fix: in BOTH the CPU loop and the GPU-path viewport cull, resolve the real extent for BlockRef via `resolved_blockref_bbox(br)` (the exact pattern `add_window_selection` already uses) instead of `e.bbox()`. CPU = fixes the micro-cull (the reported bug); GPU = fixes an off-screen-insertion-point edge case (no micro-cull there). APX/LOD dot path left as-is (approximate by design). Build clean.

### BUG FIX #2: block/insert pick RELEASE grabbed a grip (frame-timing) (2026-06-13, uncommitted)
After fix #1 (in_click_only_phase), the PRESS correctly fired the click and captured the base point — but the capture cleared `block_dialog_pick_base` on that press frame, so by the RELEASE frame `pointer_mode_idle` flipped back true and the release's `clicked()` grabbed a grip (recorder showed `grip_drag false → true` on the release, ~130 ms after the press-click). The click cascade was already protected by `pending_release_swallow` (set on the press, swallows the matching release) but the **grip-grab handler ran before that guard and ignored it**. Fix: gate the grip `try_grab` with `!self.pending_release_swallow` (still set when the grip handler runs on the release frame; the swallow bookkeeping clears it later that frame). Generalizes the cascade's release-swallow to grips for ALL press-fires-click gestures. ALSO: added `block_def_state` / `insert_state` / `block_pick_base` to the Session Recorder state-poll (`WatchedState` + `diff_watched` + `dbg_watched_now`) — the blind spot that made this diagnosis require inferring the phase from press timing. Build clean; 17 app tests pass.

### BUG FIX: block base-point / insert point pick stolen by grips (2026-06-13, uncommitted)
During the block "Pick ⊕" base-point pick (and `block <name>` base pick + `insert` insertion-point pick), clicking on a still-selected source dobject started a GRIP DRAG instead of capturing the point. Root cause: `in_click_only_phase` (which drives `pointer_mode_idle`, the gate for grip-grab + pointer-as-selector window drag) omitted `block_dialog_pick_base`, `block_def_state`, `insert_state`. So `pointer_mode_idle` was true → with `GrpEnb` on + objects selected, the press hit a grip first and the point-capture handler never ran. Fix: added those three to `in_click_only_phase` (same set added to `snap_phase_active` for the osnap fix) — now the pick is click-only: no grips, no window-drag, press fires a clean single-click straight to the base/insert handler (which also gets osnap). Build clean.

### DObject Properties dialog — single + group editing (2026-06-13, uncommitted)
Reworked the old "Info / Properties" window into a unified **DObject Properties** dialog (`render_props_body(targets)`), edits the current selection — single OR group. Inventory derived from the structs: **General** = the `Style` set (layer, color, linetype, lt-scale, lineweight, visible) shown per-field as the shared value or `*VARIES*`; editing writes to EVERY target. **Type properties** (only when the selection is homogeneous): Point PDMODE+size, Polyline closed, Hatch pattern (read-only label → use `hatch`), Wall style+thickness, Text style+height+angle (+content when single), Dimension style, BlockRef scale+rotation. **Geometry** is read-only (`describe()`) — math coordinates are NOT edited here (grips do that), per the user's spec. Each change = ONE undo step (`props_apply` snapshots once; numeric/text fields coalesce a drag/type gesture via `props_edit_gesture` + `props_gesture_snapshot`). Helpers: `shared_style`, `props_shared_geom` (f64/bool encoded to i64 for exact equality), `targets_uniform_tag`/`geom_tag`. Group color routes through the polar ACI wheel via new `AciPickRequest::DobjectMany` (unit variant; index list in `aci_pick_many` so the enum stays `Copy`). Entry points: bare `props`/`properties`/`chprop` opens it; **Modify → Properties…**; existing toolbar `info` + Tools toggles. Old `render_info_single`/`render_info_multi` deleted. Build clean; 17 app + 161 kernel tests pass.

### BUG FIX: dist/insert/block point-picks had no running osnap (2026-06-13, uncommitted)
`snap_phase_active` (the gate that decides whether `find_all_snaps` runs each frame) listed every transform op (move/copy/rotate/scale/mirror/align/stretch/break) and `tool != None`, but **omitted `dist_state`, `insert_state`, `block_def_state`** — three phases that still commit a precise coordinate via `click_world` (`click_world = snap_hit.point ?? apply_constraints(world)`). With no snap candidates gathered, `snap_hit` was always `None`, so those clicks captured CARD/grid/raw only — no END/MID/CEN/etc. Fix: added all three to `snap_phase_active`. Also added a `card_anchor` arm for `DistState::WaitingForP2(p1)` → anchor = P1, so CARD + PER/TAN work for dist's second point relative to the first (mirrors move/copy's WaitingForDest base). Build clean.
...
## 7. Block Editor + parametric blocks + door-cuts-wall (2026-06-17)

Isolated **Block Editor** (`BlockEditor` / `render_block_editor` in app.rs) — embedded canvas, own pan/zoom, main canvas gated off while open (`canvas_locked`). Replaces the explode-to-screen recorder.

- **Parametric model:** demo stretch teaches dir + which-vertices; amount is value-driven `dir·gain·(value−original)` (`block_derived_geoms`). Tasks = rows sharing a name (correlated). Per-vector `gain` (kernel `ParamVector.gain`). Measure-original (2-click), CARD lock, object snap inside the editor. Re-open seeds existing params + cut_edges (no longer drops old tasks).
- **Door-cuts-wall:** `Block.cut_edges` (jamb edges, marked via "Mark cut edges ✂"). Insert flow: point → ANGLE (`InsertState::WaitingForAngle`) → place + `apply_block_cut` clips host Line/Polyline inside the cut region (convex hull of derived+transformed cut edges).
